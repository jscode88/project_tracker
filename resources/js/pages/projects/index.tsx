...
export default function ProjectsIndex({
    ...
    const createHref = owner
        ? `/projects/create?owner_id=${owner.id}`
        : '/projects/create';

    return (
        <>
            <PageHeader
                title={owner ? `${owner.name} Projects` : 'Projects'}
                description="Project portfolio with owners, referrers, services, and payment history."
                    ...
                    <tbody>
                        {projects.data.map((project) => (
                            <tr key={project.id} className="border-t">
                                ...
                                <td className="px-4 py-3">
                                    {project.price_calculations?.length ? (
                                        <div className="flex flex-wrap gap-2">
                                            {project.price_calculations.map(
                                                (calculation) => (
                                                    <Link
                                                        key={calculation.id}
                                                        className="text-primary underline-offset-4 hover:underline"
                                ...
                                <td className="px-4 py-3">
                                    <div className="flex justify-end gap-2">
                                        ...
                                        <CrudActions
                                            ...
                                            deleteHref={`/projects/${project.id}`}
                                        />
                                    </div>
                                </td>
                            </tr>
